...
def _compute_corr_natively(df: DataFrame, summary: dict, corr_type: str) -> ArrayType:
    ...
    """
    variables = {column: {"type": description["type"], "all_missing": description["n"] - description["n_missing"] == 0} for column, description in summary.items()}
    interval_columns = [
        column for column, mapping in variables.items() if mapping["type"] == "Numeric" and not mapping["all_missing"]
    ]
